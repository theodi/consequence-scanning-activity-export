require(['bowser', 'underscore', 'backbone', 'semver', 'react', 'react-dom', 'html-react-parser', 'libraries/ckeditor', 'libraries/SCORM_API_wrapper', 'libraries/lzma-min', 'libraries/lzma_worker-min.js', 'libraries/react-beautiful-dnd.min.js'], (function (Bowser, _$1, Backbone$1, semver, React, ReactDOM, HTMLReactParser, ClassicEditor, pipwerks, lzmaMin, lzma_workerMin_js, reactBeautifulDnd_min_js) {
  ...
  const logging$1 = new Logging();
  var logging$2 = __AMD("core/js/logging", logging$1);
  ...
  class Components extends Backbone__default["default"].Controller {
    ...
    register(name, object) {
      ...
      if (!object.view && !object.model || object instanceof Backbone__default["default"].View) {
        logging$2.deprecated('View-only registrations are no longer supported');
        object = {
          view: object
        };
      }
    ...
    getViewClass(nameModelViewOrData) {
      ...
      if (!object) {
        logging$2.error(`A view for '${name}' isn't registered in your project`);
        return;
      }
    ...
    getModelName(nameModelOrData) {
      ...
      if (nameModelOrData instanceof Object) {
        ...
        if (isViewOnlyQuestion) {
          logging$2.deprecated(`Assuming a question model for a view-only question: ${name}`);
          return 'question';
        }
    ...
    getModelClass(nameModelOrData) {
      ...
      if (!object) {
        logging$2.error(`A model for '${name}' isn't registered in your project`);
        return;
      }
  ...
  var ConfigModel = __AMD("core/js/models/configModel", class ConfigModel extends LockingModel {
    ...
    initialize(attrs, options) {
      ...
      this.fetch({
        ...
        error: () => logging$2.error('Unable to load course/config.json')
      });
    }
  ...
  class Data extends AdaptCollection {
    ...
    async loadManifestFiles(languagePath) {
      ...
      try {
        ...
        logging$2.error(`Manifest path '${manifestPath} not found. Using traditional files: ${manifest.join(', ')}`);
      }
      ...
      const allModelData = allFileData.reduce((result, fileData) => {
        if (Array.isArray(fileData)) {
          ...
          logging$2.warnOnce(`File data isn't an array or object: ${fileData.__path__}`);
        }
      ...
      allModelData.forEach(modelData => {
        ...
        try {
          ...
          logging$2.error(`Failed to load object ${modelData.__path__}${Object.prototype.hasOwnProperty.call(modelData, '__index__') ? `[${modelData.__index__}]` : ''}`);
          logging$2.error(error);
          return;
        }
    ...
    async triggerDataLoaded() {
      logging$2.debug('Firing app:dataLoaded');

      try {
        this.forEach(model => model.setupModel?.());
        Adapt$1.trigger('app:dataLoaded');
      } catch (e) {
        logging$2.error('Error during app:dataLoading trigger', e);
      }
    ...
    async triggerDataReady(newLanguage) {
      ...
      logging$2.debug('Firing app:dataReady');

      try {
        Adapt$1.trigger('app:dataReady');
      } catch (e) {
        logging$2.error('Error during app:dataReady trigger', e);
      }
    ...
    logReadyError(view) {
      ...
      logging$2.error(`View ${notReadyDescendants.map(model => `${model.get('_id')} (${model.get('_component') ?? model.get('_type')})`).join(', ')} failed to become ready, forcing ready status.`);
    ...
    checkIds() {
      ...
      if (orphanedIds.length) {
        logging$2.error(`Orphaned _ids: ${orphanedIds.join(', ')}`);
      }

      if (missingIds.length) {
        logging$2.error(`Missing _ids: ${missingIds.join(', ')}`);
      }

      if (emptyIds.length) {
        logging$2.error(`Empty _ids: ${emptyIds.join(', ')}`);
      }

      if (duplicateIds.length) {
        logging$2.error(`Duplicate _ids: ${duplicateIds.join(', ')}`);
      }
    ...
    checkTrackingIds() {
      ...
      if (missingTrackingIds.length) {
        logging$2.error(`Missing _trackingIds: ${missingTrackingIds.join(', ')}`);
      }
      ...
      if (duplicateTrackingIds.length) {
        logging$2.error(`Duplicate _trackingIds: ${duplicateTrackingIds.join(', ')}`);
      }
  ...
  class A11y extends Backbone.Controller {
    ...
    _removeLegacyElements() {
      ...
      logging$2.warn('REMOVED: #accessibility-toggle and #accessibility-instructions have been removed. Please remove them from all of your .html files.');
    ...
    ariaLevel() {
      ...
      function calculateLevel() {
        ...
        if (!hasModelId) {
          logging$2.warnOnce('Cannot calculate appropriate heading level, no model id was specified');
  ...
  var AdaptModel = __AMD("core/js/models/adaptModel", class AdaptModel extends LockingModel {
    ...
    get(name) {
      switch (name) {
        case '_parent':
        case '_children':
          logging$2.deprecated('Use model.getChildren() or model.getParent() instead of model.get(\'_children\') or model.get(\'_parent\')');
      }
    ...
    isTypeGroup(typeGroup) {
      ...
      if (isPluralized || hasUpperCase) {
        logging$2.deprecated(`'${typeGroup}' appears pluralized or contains uppercase characters, suggest using the singular, lowercase type group '${singularLowerCased}'.`);
      }
    ...
    setOptional(value) {
      logging$2.deprecated('Use model.set(\'_isOptional\', value) as setOptional() may be removed in the future');
      this.set({
        _isOptional: value
      });
    }
  ...
  var ContentObjectModel = __AMD("core/js/models/contentObjectModel", class ContentObjectModel extends AdaptModel {
    get _parent() {
      logging$2.deprecated('contentObjectModel._parent, use contentObjectModel.getParent() instead, parent models are defined by the JSON');
    ...
    get _siblings() {
      logging$2.deprecated('contentObjectModel._siblings, use contentObjectModel.getSiblings() instead, sibling models are defined by the JSON');
      return 'contentObjects';
    }

    get _children() {
      logging$2.deprecated('contentObjectModel._children, use contentObjectModel.hasManagedChildren instead, child models are defined by the JSON');
      return null;
    }
  ...
  class ArticleModel extends AdaptModel {
    get _parent() {
      logging$2.deprecated('articleModel._parent, use articleModel.getParent() instead, parent models are defined by the JSON');
      return 'contentObjects';
    }

    get _siblings() {
      logging$2.deprecated('articleModel._siblings, use articleModel.getSiblings() instead, sibling models are defined by the JSON');
      return 'articles';
    }

    get _children() {
      logging$2.deprecated('articleModel._children, use articleModel.hasManagedChildren instead, child models are defined by the JSON');
      return 'blocks';
    }
  ...
  class BlockModel extends AdaptModel {
    get _parent() {
      logging$2.deprecated('blockModel._parent, use blockModel.getParent() instead, parent models are defined by the JSON');
      return 'articles';
    }

    get _siblings() {
      logging$2.deprecated('blockModel._siblings, use blockModel.getSiblings() instead, sibling models are defined by the JSON');
      return 'blocks';
    }

    get _children() {
      logging$2.deprecated('blockModel._children, use blockModel.hasManagedChildren instead, child models are defined by the JSON');
      return 'components';
    }
  ...
  class ComponentModel extends AdaptModel {
    get _parent() {
      logging$2.deprecated('componentModel._parent, use componentModel.getParent() instead, parent models are defined by the JSON');
      return 'blocks';
    }

    get _siblings() {
      logging$2.deprecated('componentModel._siblings, use componentModel.getSiblings() instead, sibling models are defined by the JSON');
      return 'components';
    }
  ...
  class MenuModel extends ContentObjectModel {
    get _children() {
      logging$2.deprecated('menuModel._children, use menuModel.hasManagedChildren instead, child models are defined by the JSON');
      return 'contentObjects';
    }
  ...
  class CourseModel extends MenuModel$1 {
    get _parent() {
      logging$2.deprecated('courseModel._parent, use courseModel.getParent() instead, parent models are defined by the JSON');
      return null;
    }

    get _siblings() {
      logging$2.deprecated('courseModel._siblings, use courseModel.getSiblings() instead, sibling models are defined by the JSON');
      return null;
    }
  ...
  class PageModel extends ContentObjectModel {
    get _children() {
      logging$2.deprecated('pageModel._children, use menuModel.hasManagedChildren instead, child models are defined by the JSON');
      return 'articles';
    }
  ...
  var ChildEvent = __AMD("core/js/childEvent", class ChildEvent extends Backbone.Controller {
    ...
    set model(model) {
      if (this.type !== 'requestChild') {
        logging$2.warn(`Cannot change model in ${this.type} event.`);
        return;
      }

      if (this._model) {
        logging$2.warn(`Cannot inject two models in one sitting. ${model.get('_id')} attempts to overwrite ${this._model.get('_id')}`);
        return;
      }
  ...
  function html(html) {
    if (!html) return;
    logging$2.deprecated('reactHelpers.html please use react dangerouslySetInnerHTML instead: https://reactjs.org/docs/dom-elements.html#dangerouslysetinnerhtml');
    return {
      __html: html
    };
  }
  ...
  class AdaptView extends Backbone.View {
    ...
    get childViews() {
      logging$2.deprecated('view.childViews use view.getChildViews() and view.setChildViews([])');
  ...
  var NotifyView = __AMD("core/js/views/notifyView", class NotifyView extends Backbone.View {
    ...
    _deprecated(type, notifyObject) {
      logging$2.deprecated(`NOTIFY DEPRECATED: Adapt.trigger('notify:${type}', notifyObject); is no longer supported, please use notify.${type}(notifyObject);`);
  ...
  class Router extends Backbone.Router {
    ...
    navigateToArguments(args) {
      ...
      logging$2.deprecated('Use Backbone.history.navigate or window.location.href instead of Adapt.trigger(\'router:navigateTo\')');
    ...
    async handleId(id) {
      ...
      if (model.get('_isLocked') && Adapt$1.config.get('_forceRouteLocking')) {
        logging$2.warn('Unable to navigate to locked id: ' + id);
        this.model.set('_canNavigate', true, {
          pluginName: 'adapt'
        });
      ...
      if (!ViewClass && isMenu) {
        logging$2.deprecated(`Using event based menu view instantiation for '${components$1.getViewName(model)}'`);
        return;
      }
    ...
    async navigateToElement(selector) {
      ...
      if (isElementUnavailable) {
        logging$2.warn(`router.navigateToElement, selector not found in document: ${selector}`);
        return;
      }
    ...
    get() {
      logging$2.deprecated('router.get, please use router.model.get');
      return this.model.get(...arguments);
    }

    set() {
      logging$2.deprecated('router.set, please use router.model.set');
  ...
  class Scrolling extends Backbone.Controller {
    ...
    async scrollTo(selector) {
      let settings = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
      logging$2.deprecated('Adapt.scrollTo and Adapt.scrolling.scrollTo, use router.navigateToElement instead.');
  ...
  class Tracking extends Backbone.Controller {
    ...
    loadConfig() {
      ...
      if (legacyShouldSubmitScore !== undefined) logging$2.deprecated('config.json:_spoor._tracking._shouldSubmitScore, please use only config.json:_completionCriteria._shouldSubmitScore');
  ...
  const beginWait = () => {
    logging$2.deprecated('Use wait.begin() as Adapt.trigger(\'plugin:beginWait\') will be removed in the future');
    wait$1.begin();
  };

  const endWait = () => {
    logging$2.deprecated('Use wait.end() as Adapt.trigger(\'plugin:endWait\') will be removed in the future');
    wait$1.end();
  };
  ...
  const ready = () => {
    ...
    logging$2.deprecated("Use wait.queue(callback) as Adapt.on('plugins:ready', callback) will be removed in the future");
  ...
  Object.defineProperties(Adapt$1, {
    ...
    a11y: {
      get() {
        logging$2.deprecated('Adapt.a11y, please use core/js/a11y directly');
        return a11y$1;
      }
    ...
    componentStore: {
      get() {
        logging$2.deprecated('Adapt.componentStore, please use core/js/components directly');
    ...
    data: {
      get() {
        logging$2.deprecated('Adapt.data, please use core/js/data directly');
        return data$1;
      }
    ...
    device: {
      get() {
        logging$2.deprecated('device, please use core/js/device directly');
        return device$1;
      }
    ...
    drawer: {
      get() {
        logging$2.deprecated('Adapt.drawer, please use core/js/drawer directly');
        return drawer;
      }
    ...
    findById: {
      get() {
        logging$2.deprecated('Adapt.findById, please use data.findById directly');
    ...
    findViewByModelId: {
      get() {
        logging$2.deprecated('Adapt.findViewByModelId, please use data.findViewByModelId directly');
    ...
    findByTrackingPosition: {
      get() {
        logging$2.deprecated('Adapt.findByTrackingPosition, please use data.findByTrackingPosition directly');
    ...
    getViewName: {
      get() {
        logging$2.deprecated('Adapt.getViewName, please use components.getViewName instead');
    ...
    getViewClass: {
      get() {
        logging$2.deprecated('Adapt.getViewClass, please use components.getViewClass instead');
    ...
    getModelName: {
      get() {
        logging$2.deprecated('Adapt.getModelName, please use components.getModelName instead');
    ...
    getModelClass: {
      get() {
        logging$2.deprecated('Adapt.getModelClass, please use components.getModelClass instead');
    ...
    location: {
      get() {
        logging$2.deprecated('Adapt.location, please use core/js/location directly');
        return location$1;
      }
    ...
    log: {
      get() {
        logging$2.deprecated('Adapt.log, please use core/js/logging directly');
        return logging$2;
      }
    ...
    mpabc: {
      get() {
        logging$2.deprecated('Adapt.mpabc, please use core/js/mpabc directly');
        return mpabc$1;
      }
    ...
    notify: {
      get() {
        logging$2.deprecated('Adapt.notify, please use core/js/notify directly');
        return notify$1;
      }
    ...
    offlineStorage: {
      get() {
        logging$2.deprecated('offlineStorage, please use src/core/offlineStorage instead');
    ...
    navigateToElement: {
      get() {
        logging$2.deprecated('Adapt.navigateToElement, please use router.navigateToElement');
    ...
    register: {
      get() {
        logging$2.deprecated('Adapt.register, please use components.register instead');
    ...
    router: {
      get() {
        logging$2.deprecated('Adapt.router, please use core/js/router directly');
        return router$1;
      }
    ...
    scrolling: {
      get() {
        logging$2.deprecated('Adapt.scrolling, please use core/js/scrolling directly');
        return scrolling$1;
      }
    ...
    scrollTo: {
      get() {
        logging$2.deprecated('Adapt.scrollTo, please use router.navigateToElement');
    ...
    startController: {
      get() {
        logging$2.deprecated('Adapt.startController, please use core/js/startController directly');
    ...
    store: {
      get() {
        logging$2.deprecated('Adapt.store, please use core/js/components directly');
        return components$1._register;
      }
    ...
    tracking: {
      get() {
        logging$2.deprecated('Adapt.tracking, please use core/js/tracking directly');
        return tracking$1;
      }
    ...
    wait: {
      get() {
        logging$2.deprecated('Adapt.wait, please use src/core/wait instead');
        return wait$1;
      }
    ...
    Adapt$1.on('reactElement:preRender', event => {
      ...
      if (hasManyChildren) {
        event.args = event.args.map(child => {
          ...
          logging$2.warnOnce(`html() call should be the only child in its parent <${tagName} ${attributes}>`);
  ...
  const helpers$1 = {
    ...
    compile_a11y_text(template, context) {
      logging$2.deprecated('a11y_text is no longer required. https://tink.uk/understanding-screen-reader-interaction-modes/');
    ...
    a11y_attrs_heading(level) {
      logging$2.deprecated('a11y_attrs_heading, please use a11y_aria_level or a11y.ariaLevel');
    ...
    a11y_attrs_tabbable() {
      logging$2.deprecated('a11y_attrs_tabbable should not be used. tabbable elements should be natively tabbable.');
  ...
  Object.assign(helpers$1, {
    if_value_equals() {
      logging$2.deprecated('if_value_equals, use equals instead.');
      return helpers$1.equals.apply(this, arguments);
    },

    numbers() {
      logging$2.deprecated('numbers, use inc instead.');
      return helpers$1.inc.apply(this, arguments);
    },

    lowerCase() {
      logging$2.deprecated('lowerCase, use lowercase instead.');
      return helpers$1.lowercase.apply(this, arguments);
    }

  });

  for (const name in helpers$1) {
    Handlebars.registerHelper(name, helpers$1[name]);
  }
  ...
  class ComponentView extends AdaptView$1 {
    ...
    renderState() {
      logging$2.removed('renderState is removed and moved to item title');
    }
  ...
  class QuestionView extends ComponentView$1 {
    ...
    isInteractive() {
      logging$2.deprecated('questionView.isInteractive please use this.model.isInteractive, this.model.get(\'_shouldShowMarking\') or this.model.shouldShowMarking');
    ...
    preRender() {
      ...
      if (this.checkIfResetOnRevisit !== QuestionView.prototype.checkIfResetOnRevisit) {
        logging$2.deprecated('QuestionView.checkIfResetOnRevisit, please use QuestionModel.checkIfResetOnRevisit');
      }
    ...
    checkIfResetOnRevisit() {
      ...
      logging$2.deprecated('QuestionView.resetQuestionOnRevisit, please QuestionModel.reset');
  ...
  class ViewOnlyQuestionViewCompatibilityLayer extends QuestionView {
    getInteractionObject() {
      logging$2.deprecated('QuestionView.getInteractionObject, please use QuestionModel.getInteractionObject');
    ...
    getResponseType() {
      logging$2.deprecated('QuestionView.getResponseType, please use QuestionModel.getResponseType');
      return this.model.getResponseType();
    }

    setupDefaultSettings() {
      logging$2.deprecated('QuestionView.setupDefaultSettings, please use QuestionModel.setupDefaultSettings');
      return this.model.setupDefaultSettings();
    }

    setupButtonSettings() {
      logging$2.deprecated('QuestionView.setupButtonSettings, please use QuestionModel.setupButtonSettings');
      return this.model.setupButtonSettings();
    }

    setupWeightSettings() {
      logging$2.deprecated('QuestionView.setupWeightSettings, please use QuestionModel.setupWeightSettings');
      return this.model.setupWeightSettings();
    }

    canSubmit() {
      logging$2.deprecated('QuestionView.canSubmit, please use QuestionModel.canSubmit');
      return this.model.canSubmit();
    }

    updateAttempts() {
      logging$2.deprecated('QuestionView.updateAttempts, please use QuestionModel.updateAttempts');
      return this.model.updateAttempts();
    }

    storeUserAnswer() {
      logging$2.deprecated('QuestionView.storeUserAnswer, please use QuestionModel.storeUserAnswer');
      return this.model.storeUserAnswer();
    }

    resetUserAnswer() {
      logging$2.deprecated('QuestionView.resetUserAnswer, please use QuestionModel.resetUserAnswer');
    ...
    isCorrect() {
      logging$2.deprecated('QuestionView.isCorrect, please use QuestionModel.isCorrect');
      return this.model.isCorrect();
    }

    setScore() {
      logging$2.deprecated('QuestionView.setScore, please use QuestionModel.setScore');
      return this.model.setScore();
    }

    updateButtons() {
      logging$2.deprecated('QuestionView.updateButtons, please use QuestionModel.updateButtons');
    ...
    isPartlyCorrect() {
      logging$2.deprecated('QuestionView.isPartlyCorrect, please use QuestionModel.isPartlyCorrect');
      return this.model.isPartlyCorrect();
    }

    setupCorrectFeedback() {
      logging$2.deprecated('QuestionView.setupCorrectFeedback, please use QuestionModel.setupCorrectFeedback');
      return this.model.setupCorrectFeedback();
    }

    setupPartlyCorrectFeedback() {
      logging$2.deprecated('QuestionView.setupPartlyCorrectFeedback, please use QuestionModel.setupPartlyCorrectFeedback');
      return this.model.setupPartlyCorrectFeedback();
    }

    setupIncorrectFeedback() {
      logging$2.deprecated('QuestionView.setupIncorrectFeedback, please use QuestionModel.setupIncorrectFeedback');
  ...
  class DndMultipleView extends QuestionView$1 {
    preinitialize() {
      var _this = this;

      this.setItems = function () {
        return _this.model.setItems(...arguments);
      };
    }
  ...
  var DndMultipleView$1 = __AMD("components/adapt-dnd-multiple/js/DndMultipleView", DndMultipleView);

  var DndMultipleModel = __AMD("components/adapt-dnd-multiple/js/DndMultipleModel", class DnDMultipleModel extends QuestionModel$1 {
    init() {
      super.init();
      this.checkCanSubmit();
      console.log(this.get('_items'));
      const openTextComponent = Adapt$1.findById(this.get('textId'));
    ...
    updateItems() {
      const updatedValuesHtml = Adapt$1.findById(this.get('textId')).getUserAnswer();
      ...
      items.forEach(item => {
        item._options.forEach((option, optionIndex) => {
          if (this.get('_useStrictOrder')) {
            option._isCorrect = option._optionIndex === optionIndex && option._itemIndex === item.id;
          } else {
            option._isCorrect = option._itemIndex === item.id;
          }
        });

        if (item._options.every(option => {
    ...
    storeUserAnswer() {
      ...
      const userAnswer = items.reduce((userAnswer, item) => {
        userAnswer.push(...item._options.map(option => {
    ...
    getDraggableItems() {
      ...
      return Object.keys(items).filter(key => key !== '0').map(key => items[key]);
    }
    ...
    isCorrect() {
      const hasAllCorrectAnswers = this.getDraggableItems().every(item => {
        return item._options.every((option, optionIndex) => {
          if (this.get('_useStrictOrder')) {
            return option._itemIndex === item.id && option._optionIndex === optionIndex;
          }
      ...
      const _numberOfCorrectAnswers = this.getDraggableItems().reduce((numberOfCorrectAnswers, item) => {
        return numberOfCorrectAnswers + item._options.filter((option, optionIndex) => {
    ...
    setScore() {
      ...
      const questionWeight = this.get('_questionWeight');
      const optionsLength = this.getDraggableItems().reduce((optionsLength, item) => {
    ...
    resetItems() {
      ...
      const items = Object.keys(this.get('_items')).map(key => objectItems[key]);
      items.forEach(item => {
        item._isCorrect = true;
        item._userAnswer = [];
        item._options = [];
  ...
  __AMD("components/adapt-dnd-multiple/js/adapt-dnd-multiple", components$1.register('dnd-multiple', {
    model: DndMultipleModel,
    view: DndMultipleView$1
  }));

  class OpenTextInputModel extends QuestionModel$1 {
    formatPlaceholder() {
    ...
    setupQuestion(localUserAnswer) {
      ...
      let _buttonState = BUTTON_STATE$1.SUBMIT;

      if (this.get('_isComplete')) {
        _buttonState = BUTTON_STATE$1.CORRECT;

        if (this.get('_canShowModelAnswer')) {
          _buttonState = BUTTON_STATE$1.SHOW_CORRECT_ANSWER;
        }
      }
    ...
    getInteractionObject() {
      return {
        correctResponsesPattern: [this.get('modelAnswer')]
      };
    }

  }
  var OpenTextInputModel$1 = __AMD("components/adapt-formatted-openTextInput/js/openTextInputModel", OpenTextInputModel);
  ...
  class OpenTextInputView extends QuestionView$1 {
    events() {
      return {
    ...
    onCompleteChanged(isComplete, buttonState) {
      ...
      Adapt$1.a11y.toggleAccessibleEnabled(this.$('.btn__action'), true);
      if (_.isEmpty(buttonState)) return;
      let _buttonState = BUTTON_STATE$1.HIDE_CORRECT_ANSWER;

      if (buttonState === BUTTON_STATE$1.CORRECT || buttonState === BUTTON_STATE$1.HIDE_CORRECT_ANSWER || buttonState === BUTTON_STATE$1.SUBMIT) {
        _buttonState = BUTTON_STATE$1.SHOW_CORRECT_ANSWER;
      }
    ...
    onQuestionRendered() {
      ...
      if (this.model.get('_isComplete') && !this.model.get('_canShowModelAnswer')) {
        this.model.set('_buttonState', BUTTON_STATE$1.CORRECT);
      }
    ...
    postRender() {
      QuestionView$1.prototype.postRender.call(this);
      ...
      this.$('.demo-update__send');
      const maxCharacters = this.model.get('_allowedCharacters');
      ClassicEditor__default["default"].create(document.querySelector('#textbox__' + this.model.get('_id')), {
        wordCount: {
          onUpdate: stats => {
    ...
    showCorrectAnswer() {
      ...
      Adapt$1.a11y.focusFirst(this.$('.opentextinput__item-modelanswer'));
    }
    ...
    hideCorrectAnswer() {
      ...
      this.$('.opentextinput__item-modelanswer').addClass(HIDE_MODEL_ANSWER_CLASS).removeClass(SHOW_MODEL_ANSWER_CLASS);
    }
  ...
  var OpenTextInputView$1 = __AMD("components/adapt-formatted-openTextInput/js/openTextInputView", OpenTextInputView);

  Adapt$1.once('adapt:start', restoreQuestionStatusPolyfill);
  __AMD("components/adapt-formatted-openTextInput/js/adapt-formatted-openTextInput", Adapt$1.register('formattedOpenTextInput', {
    view: OpenTextInputView$1,
    model: OpenTextInputModel$1
  }));

  function restoreQuestionStatusPolyfill() {
    Adapt$1.components.each(function (component) {
  ...
  class TrialogueView extends ComponentView$1 {
    preRender() {
    ...
    onClick(item, option) {
      ...
      if (option.text === 'Explore a different scenario') {
  ...
  var TrialogueView$1 = __AMD("components/adapt-trialogue/js/TrialogueView", TrialogueView);
  ...
  var TrialogueModel = __AMD("components/adapt-trialogue/js/TrialogueModel", class TrialogueModel extends ItemsComponentModel {
    restoreUserAnswers() {
      const booleanArray = this.get('_userAnswer');
      if (!booleanArray) return;
      this.getChildren().forEach((child, index) => child.set('_isVisited', booleanArray[index]));
  ...
  __AMD("components/adapt-trialogue/js/adapt-trialogue", Adapt$1.register('trialogue', {
    model: TrialogueModel,
    view: TrialogueView$1
  }));

  var aboutPage = _.extend({
    initialize: function () {
      this.listenTo(Adapt$1, 'aboutPage:showAboutPage', this.showAboutPage);
      this.listenTo(Adapt$1, 'router:course', this.updateCourse);
      this.listenTo(Adapt$1, 'pageView:ready', this.addLink);
      this.listenTo(Adapt$1, 'menuView:ready', this.addLink);
      this.listenTo(Adapt$1, 'router:page', this.updatePage);
    },
    ...
    addLink: function () {
      ...
      try {
        title = Adapt$1.course.get('_globals')._extensions._aboutPage.linkText;
      } catch (err) {}

      try {
        var items = Adapt$1.course.get('_aboutPage')._items;
      } catch (err) {
        return;
      }
      ...
      try {
        contactEmail = Adapt$1.course.get('_globals')._extensions._aboutPage.contactEMail;
        contactLinkText = Adapt$1.course.get('_globals')._extensions._aboutPage.contactLinkText;
    ...
    showAboutPage: function () {
      ...
      if (items.length < 1) {
        items = Adapt$1.course.get('_aboutPage')._items;
      }

      var title = "About";

      try {
        title = Adapt$1.course.get('_globals')._extensions._aboutPage.titleText;
      } catch (err) {}
      ...
      Adapt$1.once("notify:closed", function () {
        Adapt$1.trigger("tutor:closed");
      });
      Adapt$1.trigger('notify:popup', alertObject);
      Adapt$1.trigger('tutor:opened');
    }
  }, Backbone.Events);

  aboutPage.initialize();
  __AMD("extensions/adapt-contrib-aboutPage/js/adapt-contrib-aboutPage", aboutPage);
  ...
  const AssessmentView = {
    ...
    _onAssessmentComplete(state, model) {
      ...
      logging$2.info('assessment complete', state, model);
    },

    _onAssessmentReset(state, model) {
      if (state.id !== this.model.get('_assessment')._id) return;
      logging$2.info('assessment reset', state, model);
    },
  ...
  class QuestionBank {
    ...
    getRandomQuestionBlocks() {
      ...
      if (count > this.unusedQuestionBlocks.length) {
        logging$2.warn(`Bank ID ${this._bankId}: Attempting to display ${count} question(s), but only ${this.unusedQuestionBlocks.length} available`);
  ...
  const AssessmentModel = {
    ...
    _onDataReady() {
      ...
      if (this.get('_attemptInProgress') && questions.every(q => q.get('_isSubmitted')) && this.get('_requireCompletionOf') !== Number.POSITIVE_INFINITY) {
        ...
        logging$2.warn(`Assessment ${this.get('_id')} _attemptInProgress corrected as all questions are submitted`);
      }
    },
    ...
    _setupAssessmentData(force, callback) {
      ...
      if (!quizModels) {
        ...
        quizModels = this.getChildren().models;
        logging$2.warn('assessment: Not enough unique questions to create a fresh assessment, using last selection');
      }
    ...
    _setupBanks() {
      ...
      this.findDescendantModels('block').filter(block => block.get('_isAvailable') && block.findDescendantModels('question').length > 0).forEach(block => {
        ...
        if (isInvalidNumber) logging$2.warn(`Bank ID ${quizBankId} is not a valid number`);
        if (isOutOfBounds) logging$2.warn(`Bank ID ${quizBankId} exceeds the number of available splits (${bankSplits.length})`);
      });
  ...
  class Assessment extends Backbone.Controller {
    ...
    _onAssessmentsComplete(state) {
      ...
      if (!this._getStateByAssessmentId(assessmentId)) {
        logging$2.warn('assessments: state was not registered when assessment was created');
      }
    ...
    _handleRoute(plugin, id) {
      ...
      if (!data$1.findById(id)) {
        ...
        if (assessment) {
          ...
          logging$2.warn('Assessment not found with _id: ' + id);
          return;
        }
      }
    ...
    _addToAssessmentIdMap(id, model) {
      if (id === undefined) {
        logging$2.warn('An assessment has been registered with an undefined value for "_id"');
        return;
      }

      if (id === '') {
        logging$2.warn('An assessment has been registered with an empty value for "_id"');
      }

      if (!this._assessments._byAssessmentId[id]) {
        this._assessments._byAssessmentId[id] = model;
      } else {
        logging$2.warn('An assessment with an _id of "' + id + '" already exists!');
      }
    }
    ...
    getState() {
      ...
      if (assessmentsConfig._correctToPass === undefined) {
        logging$2.warnOnce('Assessment course config is missing _correctToPass');
      }
  ...
  var BranchingSet = __AMD("extensions/adapt-contrib-branching/js/BranchingSet", class BranchingSet {
    ...
    restore() {
      const branching = offlineStorage$1.get('b');
      if (!branching) return;
      const id = this.model.get('_id');
      if (!branching[id]) return;
      const trackingPositions = offlineStorage$1.deserialize(branching[id]);
    ...
    getNextModel() {
      ...
      if (nextModel === undefined) {
        ...
        if (!nextBranchingSet) {
          logging$2.error(`Cannot branch to a model that isn't contained in a branching set: ${nextId} from ${lastChildModel.get('_id')}`);
          return true;
        }
    ...
    saveNextModel(nextModel) {
      const branching = offlineStorage$1.get('b') || {};
      const id = this.model.get('_id');
      const trackingIds = branching[id] && offlineStorage$1.deserialize(branching[id]) || [];
      trackingIds.push(nextModel.trackingPosition);
      branching[id] = offlineStorage$1.serialize(trackingIds);
      offlineStorage$1.set('b', branching);
    }
    ...
    get isEffectivelyComplete() {
      const isEffectivelyComplete = this.branchedModels.every(childModel => {
        ...
        const areAllAvailableTrackableChildrenComplete = childModel.getChildren().filter(model => model.get('_isAvailable') && model.get('_isTrackable') !== false).every(model => model.get('_isComplete') || model.get('_isOptional'));
  ...
  class Branching extends Backbone__default["default"].Controller {
    ...
    warnForSpoorMisconfiguration() {
      ...
      logging$2.error('Branching: Spoor is misconfigured. Branching requires _spoor._tracking._shouldStoreAttempts = true');
    }
  ...
  var ComponentSerializer = __AMD("extensions/adapt-contrib-spoor/js/serializers/ComponentSerializer", class ComponentSerializer extends Backbone.Controller {
    ...
    async serialize(shouldStoreResponses, shouldStoreAttempts) {
      if (shouldStoreAttempts && !shouldStoreResponses) {
        logging$2.warnOnce('SPOOR configuration error, cannot use \'_shouldStoreAttempts\' without \'_shouldStoreResponses\'');
      }
    ...
    deserialize(binary) {
      ...
      states.forEach(state => {
        ...
        if (!component) {
          logging$2.warn(`SPOOR could not restore tracking id: ${trackingId}, index: ${index}`);
          return;
        }
  ...
  var StatefulSession = __AMD("extensions/adapt-contrib-spoor/js/adapt-stateful-session", class StatefulSession extends Backbone.Controller {
    ...
    printCompletionInformation(suspendData) {
      ...
      if (!trackingIds.length) {
        logging$2.info(`course._isComplete: ${courseComplete}, course._isAssessmentPassed: ${assessmentPassed}, ${this._trackingIdType} completion: no tracking ids found`);
        return;
      }
      ...
      logging$2.info(`course._isComplete: ${courseComplete}, course._isAssessmentPassed: ${assessmentPassed}, ${this._trackingIdType} completion: ${completionString}`);
    }
        ...
        case COMPLETION_STATE$1.PASSED:
          {
            if (!config?._reporting?._onTrackingCriteriaMet) {
              logging$2.warn(`No value defined for '_onTrackingCriteriaMet', so defaulting to '${completionStatus}'`);
            } else {
        ...
        case COMPLETION_STATE$1.FAILED:
          {
            if (!config?._reporting?._onAssessmentFailure) {
              logging$2.warn(`No value defined for '_onAssessmentFailure', so defaulting to '${completionStatus}'`);
            } else {
  ...
  function logTrickleState() {
    if (logging$2._config?._level !== 'debug') return;

    if (!Adapt$1.parentView?.model?.isTypeGroup('page')) {
      logging$2.debug('TRICKLE GLOBAL STATE');
      Adapt$1.course.getAllDescendantModels(true).filter(model => model.get('_isAvailable')).forEach(model => {
        ...
        const isTrickled = model.get('_isTrickled');
        logging$2.debug(`${' '.repeat(model.getAncestorModels().length)}${model.get('_type')} ${model.get('_id')} isLocked: ${isLocked} isTrickled: ${isTrickled}`);
      });
      return;
    }

    logging$2.debug('TRICKLE STATE');
  ...
  class CourseNotesView extends Backbone__default["default"].View {
    className() {
      return 'coursenotes';
    }
  ...
  var CourseNotesView$1 = __AMD("extensions/adapt-courseNotes/js/CourseNotesView", CourseNotesView);
  ...
  class CourseNotesNavView extends Backbone.View {
    ...
    attributes() {
      return {
        'aria-label': Adapt$1.course.get('_globals')?._extensions?._courseNotes?.buttonNav || 'Open Course Notes'
      };
    }
    ...
    onClick(event) {
      if (event && event.preventDefault) event.preventDefault();
      Adapt$1.trigger('courseNotes:open');
    }

  }

  CourseNotesNavView.template = 'courseNotesNav';
  var CourseNotesNavView$1 = __AMD("extensions/adapt-courseNotes/js/CourseNotesNavView", CourseNotesNavView);

  class CourseNotes extends Backbone__default["default"].Controller {
    initialize() {
    ...
    onInitialize() {
      ...
      const navView = new CourseNotesNavView$1({
        model: new Backbone__default["default"].Model(Adapt$1.course.get('_courseNotes'))
      });
    ...
    openCourseNotes() {
      ...
      this.popupView = new CourseNotesView$1({
        model: new Backbone__default["default"].Model(Adapt$1.course.get('_courseNotes'))
      });
      notify$1.popup({
        _attributes: {
          'data-adapt-id': 'coursenotes'
        },
    ...
    checkIsEnabled() {
      const _model = Adapt$1.course.get('_courseNotes');
  ...
  __AMD("extensions/adapt-courseNotes/js/adapt-courseNotes", new CourseNotes());

  const DevtoolsModel = Backbone.Model.extend({
    initialize: function () {
      const config = Adapt$1.config.has('_devtools') ? Adapt$1.config.get('_devtools') : this.getDefaultConfig();
      this.set(_.extend({
  ...
  const Hinting = _.extend({
    ...
    toggleHints: function () {
      const contentObject = Adapt$1.findById(Adapt$1.location._currentId);
  ...
  const AutoAnswer = _.extend({
    ...
    answerMultipleChoice: function (view, isGraphical) {
      ...
      const noCorrectOptions = _.where(items, {
    ...
    answerMultipleChoiceIncorrectly: function (view, isGraphical) {
      ...
      const selectionStates = _.times(itemCount, function () {
        return false;
      });

      const nShould = _.where(items, {
        _shouldBeSelected: true
      }).length;
    ...
    answerMatching: function (view) {
      _.each(view.model.get('_items'), function (item, itemIndex) {
        const noCorrectOptions = _.where(item._options, {
          _isCorrect: true
        }).length == 0;
    ...
    answerMatchingIncorrectly: function (view) {
      const items = view.model.get('_items');
      const itemCount = items.length;

      const nIncorrect = _.random(1, itemCount);

      const selectionStates = _.shuffle(_.times(itemCount, function (i) {
        return i < nIncorrect;
      }));
    ...
    answerSliderIncorrectly: function (view) {
      const correctAnswer = view.model.get('_correctAnswer');
      const start = view.model.get('_scaleStart');
      const end = view.model.get('_scaleEnd');

      const incorrect = _.times(end - start + 1, function (i) {
        return start + i;
      });
    ...
    answerPpq: function (view) {
      ...
      const itemCount = items.length;
      const isDesktop = Adapt$1.device.screenSize != 'small';
    ...
    answerPpqIncorrectly: function (view) {
      ...
      const correct = _.shuffle(_.times(itemCount, function (i) {
        return i;
      }));
  ...
  const PassHalfFail = _.extend({
    ...
    _completeNonQuestions: function () {
      const currentModel = Adapt$1.findById(Adapt$1.location._currentId);

      const nonQuestions = _.filter(currentModel.findDescendantModels('components'), function (m) {
    ...
    half: function (callback) {
      const notSubmitted = function (view) {
    ...
    onKeypress: function (e) {
      ...
      const perform = function (type) {
        ...
        const tutorEnabled = Adapt$1.devtools.get('_feedbackEnabled');
  ...
  const ToggleBanking = {
    ...
    getBankedAssessmentsInCurrentPage: function () {
      const pageModel = Adapt$1.findById(Adapt$1.location._currentId);
    ...
    toggle: function () {
      ...
      const isBankingEnabled = function (m) {
  ...
  const MapView = Backbone.View.extend({
    ...
    onLinkClicked: function (e) {
      ...
      const model = Adapt$1.findById(id);
      e.preventDefault();
    ...
    navigateAndDisableTrickleUpTo: function (id) {
      ...
      if (model._siblings === 'contentObjects') {
        ...
        const siblings = level.getParent().getChildren();
        let sibling = null;
    ...
    navigateAndDisableTrickle: function (id) {
      ...
      if (model._siblings === 'contentObjects') {
        ...
        if (Adapt$1.location._currentId === model.findAncestor('contentObjects').get('_id')) {
          ...
          const targetModel = Adapt$1.trickle ? model.findAncestor('contentObjects') : Adapt$1.course;
  ...
  const CourseMap = _.extend({
    initialize: function () {
      const courseId = Adapt$1.course.get('_id');
  ...
  function onKeypress$2(e) {
    ...
    if (mouseTarget$1) {
      if (char == 'm') {
        const model = Utils.getModelForElement(mouseTarget$1);
  ...
  function findLabel($element) {
    ...
    return computeAccesibleName$1($label, true);
  }
  ...
  function followId($element, property) {
    ...
    return computeAccesibleName$1($toElement, true);
  }

  function computeAccesibleName$1($element) {
    let allowText = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  ...
  function computeAccessibleDescription$1($element) {
    const describedByText = followId($element, 'aria-describedby');
    if (describedByText) return describedByText;
    return '';
  }

  function getAnnotationPosition$1($element, $annotation) {
  ...
  var helpers = __AMD("extensions/adapt-devtools/js/helpers", {
    computeAccesibleName: computeAccesibleName$1,
    computeAccessibleDescription: computeAccessibleDescription$1,
    computeHeadingLevel,
    getAnnotationPosition: getAnnotationPosition$1
  });
  ...
  const AltText = _.extend({
    ...
    connectObserver: function () {
      if (this.observer) {
        this.observer.observe(document.querySelector('html'), {
          ...
          subtree: true,
          attributes: true,
          attributeFilter: ['class', 'style', 'aria-label', 'aria-labelledby', 'alt', 'aria-describedby', 'for', 'aria-valuetext', 'aria-valuenow', 'tabindex', 'aria-hidden']
        });
      }
    ...
    disconnectObserver: function () {
      ...
      this.stopListening(Adapt$1, 'popup:closed notify:closed drawer:closed', this.onDomMutation);
    ...
    render: function () {
      ...
      const $labelled = $(['.aria-label', '[alt]', '[aria-label]', '[aria-labelledby]', '[aria-describedby]', '[aria-activedescendant]', '[aria-valuetext]', '[aria-valuenow]', '[role=listbox]', '[aria-hidden]'].join(','));
      $labelled.filter(function (index, element) {
        return !$(element).parents().filter($headings).length;
      }).add($headings).each(_.bind(function (index, element) {
        const $element = $(element);
        ...
        if (!isOutOfDom && (isVisible || isHeadingHeightZero) && (isNotAriaHidden || !isAriaHidden && !isParentAriaHidden || isImg && !isParentAriaHidden)) {
          if (!annotation) this.addAnnotation($element, allowText);else this.updateAnnotation($element, annotation, allowText);
        } else if (annotation) {
          this.removeAnnotation($element, annotation);
        }
      }, this));
  ...
  function complete(element) {
    const model = Utils$1.getModelForElement(element) || Adapt$1.findById(Adapt$1.location._currentId);
    if (!model) return;
  ...
  function reset(element) {
    const model = Utils$1.getModelForElement(element);
    if (!model) return;
    const descendantComponents = model.findDescendantModels('components');
  ...
  const DevtoolsView = Backbone.View.extend({
    ...
    render: function () {
      const data = Adapt$1.devtools.toJSON();
    ...
    _checkBankingVisibility: function () {
      ...
      const bankedAssessments = ToggleBanking$1.getBankedAssessmentsInCurrentPage();
    ...
    _checkCompletePageVisibility: function () {
      ...
      const incomplete = currentModel.findDescendantModels('components', {
        where: {
          _isInteractionComplete: false
        }
      });
    ...
    _checkPassHalfFailVisibility: function () {
      ...
      const unanswered = currentModel.findDescendantModels('components', {
        where: {
          _isQuestionType: true,
          _isSubmitted: false
        }
  ...
  data$1.on('loaded', async () => {
    const isDescendant = (model, ancestor) => {
      ...
      while (parent = data$1._byAdaptID[model.get('_parentId')]) {
        if (parent === ancestor) {
          return true;
        }
    ...
    const removeDescendants = ancestor => {
      ...
      for (const id in data$1._byAdaptID) {
        const model = data$1._byAdaptID[id];
      ...
      prunable.forEach(model => data$1.remove(model));
    };
    ...
    try {
      const devConfig = await data$1.getJSON('dev.json');
  ...
  class GraphicExpandView extends Backbone.View {
    ...
    attributes() {
      const attributes = {
        'data-component': this.model.get('_component')
      };
      return attributes;
    }
  ...
  var GraphicExpandView$1 = __AMD("extensions/adapt-graphicExpand/js/graphicExpandView", GraphicExpandView);
    ...
    onComponentPostRender(view) {
      ...
      if ($insertElement.length > 1) {
        _.each($insertElement, element => {
          const graphicExpandView = new GraphicExpandView$1({
            model: model
          });
  ...
  __AMD("extensions/adapt-graphicExpand/js/adapt-graphicExpand", new GraphicExpand());

  var footerLink = _.extend({
    initialize: function () {
      this.listenTo(Adapt$1, 'router:course', this.updateCourse);
      this.listenTo(Adapt$1, 'pageView:ready', this.addLinks);
      this.listenTo(Adapt$1, 'menuView:ready', this.addLinks);
      this.listenTo(Adapt$1, 'router:page', this.updatePage);
    },
    updateCourse: function (target) {
      this.currentCourse = target;
    ...
    addLinks: function () {
      ...
      if (items.length < 1) {
        items = Adapt$1.course.get('_FooterLink')._items;
      }
  ...
  __AMD("extensions/adapt-odi-footerLink/js/adapt-odi-footerLink", footerLink);

  var licencePage = _.extend({
    initialize: function () {
      this.listenTo(Adapt$1, 'licencePage:showLicencePage', this.showLicencePage);
      this.listenTo(Adapt$1, 'router:course', this.updateCourse);
      this.listenTo(Adapt$1, 'pageView:ready', this.addLink);
      this.listenTo(Adapt$1, 'menuView:ready', this.addLink);
      this.listenTo(Adapt$1, 'router:page', this.updatePage);
    ...
    addLink: function () {
      ...
      try {
        title = Adapt$1.course.get('_globals')._extensions._ODILicence.linkText;
    ...
    showLicencePage: function () {
      ...
      if (items.length < 1) {
        items = Adapt$1.course.get('_ODILicence')._items;
      }

      var title = "Licence";

      try {
        title = Adapt$1.course.get('_globals')._extensions._ODILicence.titleText;
      } catch (err) {}
  ...
  __AMD("extensions/adapt-odi-licence/js/adapt-odi-licence", licencePage);
  ...
  var ThemeView$1 = __AMD("theme/adapt-theme-odi/js/themeView", ThemeView);
  ...
  var ThemePageView$1 = __AMD("theme/adapt-theme-odi/js/themePageView", ThemePageView);
  ...
  var ThemeArticleView$1 = __AMD("theme/adapt-theme-odi/js/themeArticleView", ThemeArticleView);
  ...
  var ThemeBlockView$1 = __AMD("theme/adapt-theme-odi/js/themeBlockView", ThemeBlockView);
  ...
  register("dndMultiple", function DndMultiple(props) {
    const {
      _isEnabled,
    ...
    const [dragItems, setDragItems] = React.useState(_items);
    ...
    const onDragEnd = _ref => {
      ...
      if (start === end) {
        ...
        setDragItems(state => ({ ...state,
          [newCol.id]: newCol
        }));
        return null;
      } else {
        const newStartList = start._options.filter((_, idx) => idx !== source.index);
    ...
    return React__default["default"].createElement("div", {
      className: "component__inner dnd-multiple__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "dnd-multiple__widget component__widget"
    }, React__default["default"].createElement(reactBeautifulDnd_min_js.DragDropContext, {
      onDragEnd: onDragEnd
    }, Object.values(dragItems).map(col => React__default["default"].createElement(templates.dndMultipleDroppable, _extends({}, props, {
      col: col,
      key: col.id,
  ...
  register("dndMultipleDraggable", function DndMultipleDragable(_ref) {
    let {
      title,
      _graphic,
      index,
    ...
    const DragItemGraphic = () => {
      ...
      return React__default["default"].createElement(templates.image, _extends({}, _graphic, {
        classNamePrefixes: ['dnd-multiple__dragitem'],
        attributionClassNamePrefixes: ['component', 'dnd-multiple']
      }));
    };

    return React__default["default"].createElement(reactBeautifulDnd_min_js.Draggable, {
      draggableId: title,
      index: index,
      isDragDisabled: !isEnabled
    }, provided => React__default["default"].createElement("div", _extends({
      className: classes(['dnd-multiple__dragitem', _graphic.isBackground && 'has-graphic-background', isCorrect && 'is-correct', !isCorrect && isCorrect !== undefined && 'is-incorrect']),
      ref: provided.innerRef
    }, provided.draggableProps, provided.dragHandleProps), DragItemGraphic(), React__default["default"].createElement("span", {
      className: "dnd-multiple__dragitem__title",
      dangerouslySetInnerHTML: {
        __html: title
      }
    }), React__default["default"].createElement("div", {
      className: "dnd-multiple__dragitem__state"
    }, React__default["default"].createElement("div", {
  ...
  register("dndMultipleDroppable", function DndMultipleDroppable(_ref) {
    let {
      col: {
        _isCorrect,
        _options,
    ...
    return React__default["default"].createElement(React__default["default"].Fragment, null, React__default["default"].createElement(reactBeautifulDnd_min_js.Droppable, {
      droppableId: droppableId
    }, provided => React__default["default"].createElement("div", {
      className: classes(['dnd-multiple__items', `dnd-multiple__items-${droppableId}`, _options.length === 0 && 'is-empty', _isCorrect && 'is-correct', !_isCorrect && _isCorrect !== undefined && 'is-incorrect']),
      style: {
        width
      }
    }, React__default["default"].createElement("div", {
      ...
      ref: provided.innerRef
    }), _options.map((option, index) => React__default["default"].createElement(templates.dndMultipleDraggable, {
      key: option.id,
      title: option.title,
  ...
  register("trialogue", function Trialogue(props) {
    const {
      _items,
      onClick
    } = props;
    const [messagesHistory, setMessageHistory] = React.useState([]);
    ...
    return React__default["default"].createElement("div", {
      className: "component__inner trialogue__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "component__widget trialogue__widget"
    }, React__default["default"].createElement(templates.trialogueMessagesHistory, {
      messages: messagesHistory
    }), React__default["default"].createElement(templates.trialogueMessage, {
      message: message,
      innerRef: messageRef,
      isAnim: isAnim
    }), React__default["default"].createElement(templates.trialogueBottomBar, {
  ...
  register("trialogueBottomBar", function trialogueBottomBar(_ref) {
    let {
      activeItem,
      onClick,
      isAnim
    ...
    return React__default["default"].createElement("div", {
      className: "trialogue__bottom-bar"
    }, React__default["default"].createElement("div", {
      className: "trialogue__bottom-bar__inner"
    }, React__default["default"].createElement("div", {
      className: "trialogue__bottom-bar__buttons"
    }, activeItem?.next === 'feedback' && React__default["default"].createElement("button", {
      className: classes(['btn-text', isAnim && 'is-disabled']),
      disabled: isAnim,
      onClick: () => onClick(activeItem, {
        text: 'restart'
      })
    }, "start again"), activeItem?.next === 'item' && activeItem?._options.map((option, index) => React__default["default"].createElement("button", {
      className: classes(['btn-text', isAnim && 'is-disabled']),
      key: index,
      disabled: isAnim,
  ...
  register("trialogueMessage", function trialogueMessage(_ref) {
    let {
      message,
      innerRef,
      isAnim
    ...
    return React__default["default"].createElement("div", {
      className: classes(['trialogue__message', sender === 'guest' ? 'trialogue__message-guest' : sender === '' ? 'trialogue__message-meta' : 'trialogue__message-host']),
      ref: innerRef
    }, React__default["default"].createElement("div", {
      className: "trialogue__message__inner"
    }, text && !isAnim && React__default["default"].createElement("div", {
      className: "trialogue__message__text"
    }, React__default["default"].createElement("div", {
      className: "trialogue__message__text-inner",
      dangerouslySetInnerHTML: {
        __html: compile(text)
      }
    })), text && isAnim && React__default["default"].createElement("div", {
      className: "trialogue__message__anim"
    }, React__default["default"].createElement("div", {
      className: "wave"
    }, React__default["default"].createElement("span", {
      className: "dot"
      ...
      className: "trialogue__message-sender"
    })));
  });

  register("trialogueMessagesHistory", function trialogueMessagesHistory(_ref) {
    let {
      messages
    } = _ref;
    return React__default["default"].createElement("div", {
      className: "trialogue__messages-history"
    }, React__default["default"].createElement("div", {
      className: "trialogue__messages-history__inner"
    }, messages.map(item => React__default["default"].createElement("div", {
      key: item._index
    }, React__default["default"].createElement("div", {
      className: classes(['trialogue__messages-history', item.sender === 'guest' ? 'trialogue__message-guest' : item.sender === '' ? 'trialogue__message-meta' : 'trialogue__message-host'])
    }, React__default["default"].createElement("div", {
      className: "trialogue__messages-history__inner"
    }, React__default["default"].createElement("div", {
      className: "trialogue__messages-history__content__text"
    }, React__default["default"].createElement("div", {
      className: "trialogue__messages-history__text-inner",
      dangerouslySetInnerHTML: {
        __html: compile(item.text)
      }
    })), React__default["default"].createElement("div", {
      className: "trialogue__messages-history-sender"
    }))), item?._options?.map(_ref2 => {
      let {
        text,
        sender,
        _index,
      ...
      return _isSelected && text !== 'start again' && React__default["default"].createElement("div", {
        key: _index,
        className: "trialogue__messages-history trialogue__message-guest"
      }, React__default["default"].createElement("div", {
        className: "trialogue__messages-history__inner"
      }, React__default["default"].createElement("div", {
        className: "trialogue__messages-history__text"
      }, React__default["default"].createElement("div", {
        className: "trialogue__messages-history__text-inner",
        dangerouslySetInnerHTML: {
          __html: compile(text)
  ...
  register("courseNotes", function CourseNotes(props) {
    ...
    const [valueTextArea, setValueTextArea] = React.useState(storage);
    const [isSaved, setIsSaved] = React.useState(true);
    const [wasChanged, setWasChanged] = React.useState(false);
    const saveNotes = React.useCallback(_.debounce(valueTextArea => {
    ...
    return React__default["default"].createElement("div", {
      className: "coursenotes__header"
    }, displayTitle && React__default["default"].createElement("div", {
      className: "coursenotes__title"
    }, React__default["default"].createElement("div", {
      className: "coursenotes__title-inner",
      dangerouslySetInnerHTML: {
        __html: compile(displayTitle, props)
      }
    })), instruction && React__default["default"].createElement("div", {
      className: "coursenotes__instruction"
    }, React__default["default"].createElement("div", {
      className: "coursenotes__instruction-inner",
      dangerouslySetInnerHTML: {
        __html: compile(instruction, props)
      }
    })), React__default["default"].createElement("div", {
      className: "coursenotes__widget"
    }, wasChanged && React__default["default"].createElement("div", {
      className: `coursenotes__status icon ${isSaved ? 'icon-tick' : 'icon-ellipsis'}`
  ...
  register("courseNotesNav", function CourseNotesNav(props) {
    return React__default["default"].createElement("span", {
      className: "icon"
    });
  });

  register("graphic-expand", function GraphicExpand(props) {
    const {
      _graphicExpand,
      onClick
    } = props;
    return React__default["default"].createElement("button", {
      onClick: onClick,
      className: "button",
      "aria-label": _graphicExpand._button.ariaLabel
    }, React__default["default"].createElement("div", {
      className: classes(['icon-expand-graphic', 'icon', _graphicExpand._button.iconClass])
    }), _graphicExpand._button.buttonText);
  });

  $('body').append(Handlebars.templates.loading());
  data$1.on('ready', () => {
    logging$2.debug('Calling Adapt.init');
    Adapt$1.init();
